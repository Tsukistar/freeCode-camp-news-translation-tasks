...
欢迎阅读我的 Git 全面指南，Git 是一种分布式版本控制系统，它彻底改变了软件开发中的协作和代码管理。
...
在阅读完这本指南后，你将对 Git 的核心概念有一个扎实的理解，并能自信、熟练地将其运用到你的开发工作流中。

## 前提条件：

你只需要带上好奇心和渴望学习的心态。本指南是为初学者精心制作的，因此不需要任何版本控制系统或编程的前置知识储备。无论你是完全的新手还是有一些编码经验，你都会发现本教程易于理解和跟着去实践。
...
使用 Git 时的大部分操作都不需要连接到服务器。因为你的电脑上拥有整个项目的历史记录，所以操作速度非常快。你可以浏览项目历史记录或查看版本之间的更改，而无需等待服务器响应。
...
理解 Git 的三种状态：已修改、已暂存和已提交，是有效进行版本控制的关键：
...
首次配置 Git 需要根据个人喜好配置 Git 环境。不过，首先你需要从 [Git - 下载][27] 或使用 Chocolatey 包下载安装 Git。然后只需按照安装说明进行操作，你就能开始使用了。

### Git 配置

我们使用 `git config` 工具来自定义 Git 环境。该工具允许我们检索和设置控制 Git 运行方式的配置变量。这些变量存储在三个不同的位置（以Linux系统为例）：
...
Git 中的身份信息用于正确地标记提交。让我们来设置您的用户名和电子邮件地址。
...
在配置您的身份之后，设置 Git 中的默认文本编辑器也非常重要。这个文本编辑器将会在 Git 需要您输入消息（如编写提交消息或解决合并冲突时）被使用。
...
顺便一提，这些参数 `"-multiInst -notabbar -nosession -noPlugin"` 用于自定义 Notepad++ 在被 Git 启动时的行为。
...
要开始使用 Git，通常需要获取一个 Git 仓库。开始做这件事有两种主要的方法：
...
通常，项目中有些文件或目录不需要被 Git 跟踪，例如日志文件、构建工件或敏感信息（如本地环境设置，如 \*.env 或 config.json）。你可以使用 `.gitignore` 文件指定要忽略的这些文件。
...
如果你过早提交了更改，或者需要对上次提交进行其他更改，请使用以下命令：

```bash
$ git commit --amend
```

这将打开提交信息编辑器，允许你修改信息。如果自上次提交以来没有任何更改，则仅允许您编辑提交信息。

****注意****: 建议只修改“仍存在于本地且尚未推送的提交”，以避免给其他合作者带来问题。
...
Git 的分支机制提供了一种强大的项目代码库管理方式，允许并行开发和实验，而不会影响主代码库。
